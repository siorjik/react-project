...
export default class Groups extends Component {
	...
	showGroupDesc(item, index, ev) {
		let arrOrder = ReactDOM.findDOMNode(this).getElementsByClassName("group");
  ...
  render() {
  	let orderDescCont = (
		...
		let orderDesc;
		if(this.state.desc) orderDesc = orderDescCont;
		else orderDesc = null;
    	...
    	<StyleRoot id="order-wrap">
	      <div style={styles.bounceInUp}>
	      	...
	      	{this.state.orders.map((item, index)=>{
	      		return <p className='group' key={item.id} onClick={this.showGroupDesc.bind(this, item, index)}><span className='order-title'>{item.title}</span> <span className='count-produts'>{item.getProducts().length}<br/>Продукта</span> <span className='date'>{item.date}</span> <span className='sum-order'>{this.getSums(item).usdSum} (USD)<br/>{this.getSums(item).uahSum} (UAH)</span> <span className='recicle'><img src={recicle} alt="rbin" title="удалить" onClick={this.deleteOrder.bind(this, item)}/></span></p>
	      	})}
	      	{orderDesc}
	      </div>
      </StyleRoot>
